  ...
  return `<main id="main" class="demo-page">
    <section aria-labelledby="demo-title">
      ...
      <div class="game-board-wrap demo-board-wrap">
        <section class="game-board" aria-labelledby="sample-game-title"><h2 id="sample-game-title">Star Signal</h2><p class="round-message" aria-live="polite">${complete ? 'Sample round complete. Both players rebuilt the signal.' : `${demoProgress} of 6 symbols are complete. It is Alex’s turn.`}</p><div class="clue-strip" role="group" aria-label="Alex’s half of the sample signal">${clues.map((clue, index) => `<div class="clue ${clue === 'partner' ? 'partner' : ''} ${index < demoProgress ? 'done' : ''}" aria-label="Step ${index + 1}: ${clue === 'partner' ? 'Sam’s clue' : clue}">${clue === 'partner' ? 'Sam' : SYMBOLS[clue]}</div>`).join('')}</div><div class="progress-pips" role="progressbar" aria-label="${demoProgress} of 6 symbols complete" aria-valuemin="0" aria-valuemax="6" aria-valuenow="${demoProgress}" aria-valuetext="${demoProgress} of 6 symbols complete">${sequence.map((_, index) => `<span class="pip ${index < demoProgress ? 'lit' : ''}"></span>`).join('')}</div>${complete ? '<button class="primary" data-action="reset-demo">Reset sample round</button>' : '<button class="primary" data-action="demo-turn">Choose drop for Alex</button>'}</section>
...
function starBoard(mine: boolean): string {
  ...
  return `<div class="clue-strip" role="group" aria-label="Your half of the signal">${clues.map((clue, index) => `<div class="clue ${clue === 'partner' ? 'partner' : ''} ${index < progress ? 'done' : ''}" aria-label="Step ${index + 1}: ${clue === 'partner' ? "partner's clue" : clue}">${clue === 'partner' ? 'other' : SYMBOLS[clue]}</div>`).join('')}</div><div class="progress-pips" role="progressbar" aria-label="${progress} of 6 symbols complete" aria-valuemin="0" aria-valuemax="6" aria-valuenow="${progress}" aria-valuetext="${progress} of 6 symbols complete">${sequence.map((_, index) => `<span class="pip ${index < progress ? 'lit' : ''}"></span>`).join('')}</div><div class="symbol-buttons" role="group" aria-label="Choose the next symbol">${Object.entries(SYMBOLS).map(([name, symbol]) => `<button class="symbol-button" data-symbol="${name}" aria-label="Choose ${name}" ${mine ? '' : 'disabled'}>${symbol}</button>`).join('')}</div>`;
}
